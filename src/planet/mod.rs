...
glium::implement_vertex!(CellData,height,vegetation);
...
impl PlanetCells{
    fn new(vertices:&Vec<glm::Vec3>)->PlanetCells{
        ...
        vertices.iter()
            .map(
                |v|
                octive_noise(perlin, &v, 2.5, 10, 0.6, 2.5)
            )
            .collect();
        ...
        vertices.iter()
            .map(
                |v|
                octive_noise(perlin, &v, 1.0, 10, 0.6, 2.5)
            )
            .collect();

        PlanetCells{
            height,
            temperature: vec![0.0;vertices.len()],
            humidity: veg
        }
    }
    ...
    fn get_cell_data(&self)->Vec<CellData>{
        self.height.iter()
            ...
            .map(|c| CellData { height: *c.0, vegetation: *c.1 })
            .collect()
    }
}
